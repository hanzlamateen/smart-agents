...
class BaseComputerTool:
    ...
    async def screenshot(self):
        ...
        # If running via SSH, we must execute command there and download
        if self.ssh_client:
            # 1. Take screenshot on remote
            # We assume worker image has scrot installed
            filename = path.name
            remote_path = f"/tmp/{filename}"
            screenshot_cmd = f"{self._display_prefix}scrot -f {remote_path} -p"
            ...
            if self._scaling_enabled:
                 ...
                 await self.shell(f"convert {remote_path} -resize {x}x{y}! {remote_path}", take_screenshot=False)

            # 2. Download via SFTP
            try:
                async with self.ssh_client.start_sftp_client() as sftp:
                    await sftp.get(remote_path, str(path))
                    # Cleanup remote
                    await self.ssh_client.run(f"rm {remote_path}")
            except Exception as e:
                raise ToolError(f"SFTP Screenshot failure: {e}")
        else:
             raise ToolError("SSH client required for screenshots")
    ...
    async def shell(self, command: str, take_screenshot=True) -> ToolResult:
        ...
        if self.ssh_client:
             ...
             raise ToolError("SSH client required for computer tool")
