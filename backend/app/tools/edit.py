...
class EditTool20250124(BaseAnthropicTool):
    ...
    async def view(self, path: Path, view_range: list[int] | None = None):
        ...
        if path.is_dir():
            ...
            stdout, stderr, _ = await self.execute_command(
        ...
        try:
            if self.ssh_client:
                async with self.ssh_client.start_sftp_client() as sftp:
                    async with sftp.open(str(path), 'r') as f:
                         return await f.read()
            else:
                raise ToolError("SSH client required for file reading")
    ...
    async def write_file(self, path: Path, file: str):
        ...
        try:
            if self.ssh_client:
                 async with self.ssh_client.start_sftp_client() as sftp:
                      async with sftp.open(str(path), 'w') as f:
                           await f.write(file)
            else:
                raise ToolError("SSH client required for file writing")
        except Exception as e:
